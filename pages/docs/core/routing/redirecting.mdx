...
<Tabs items={['TypeScript', 'JavaScript']} defaultIndex="1">
  <Tabs.Tab>
    ```tsx filename="src/app/redirect.tsx" {14-23} showLineNumbers copy
    import React from "react";
  ...
  <Tabs.Tab>
    ```tsx filename="src/app/redirect.jsx" {14-23} showLineNumbers copy
    import React from "react";
...
<AlertBox type="tip">
  The `loader` method is not only used for redirections, but it can also be used
  to perform server-side operations like making API calls, checking if the user
  is authenticated, etc. and it can pass data to the `PageComponent.render`
  method as props. Read more about it
...
<Tabs items={['TypeScript', 'JavaScript']}>
  <Tabs.Tab>
    ```tsx filename="src/app/redirect.tsx" {14} showLineNumbers copy
    import React from "react";
  ...
  <Tabs.Tab>
    ```tsx filename="src/app/redirect.jsx" {14} showLineNumbers copy
    import React from "react";
    import { PageComponent, defineRoutePage } from "rasengan";
