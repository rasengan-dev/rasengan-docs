...
The `PageComponent.loader` method is a special method used for `SSR` (Server-Side Rendering) like `getServerSideProps` in `Next.js`.
...
<Tabs items={['TypeScript', 'JavaScript']} defaultIndex="1">
  <Tabs.Tab>
    ```tsx filename="src/pages/redirect.tsx" {2,5,11} showLineNumbers copy
    import React from "react";
  ...
  <Tabs.Tab>
    ```tsx filename="src/pages/redirect.jsx" {2,5,11} showLineNumbers copy
    import React from "react";
