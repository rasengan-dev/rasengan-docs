...
<AlertBox type="tip">
  The `loader` function is not only used for redirections, but it can also be used
  to perform server-side operations like making API calls, checking if the user
  is authenticated, etc. and it can pass data to the `PageComponent` as props. Read more about it
