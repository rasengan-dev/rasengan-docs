...
<AlertBox>
Instead of manually install and configure tailwind, when you generate a new project by using the [`Rasengan CLI`](/docs/api-reference/cli) you have the possibility to use the `tailwind template` directly.
</AlertBox>
...
Add the [Tailwind CSS directives](https://tailwindcss.com/docs/functions-and-directives#directives) that `Tailwind` will use to inject its generated styles to a [`Global Stylesheet`](/docs/core/styling/css-module#stylesindexcss) in your application, for example:
      ...
      render() {
        return (
          <div className="w-full px-8 py-4">
            <h1 className="text-3xl font-bold capitalize">Home</h1>
          </div>
        )
      }
    }
