...
Finally, create the entry file called `main.ts` inside the `src` folder.
...
Learn more about [`main.ts`](/docs/api/file-convention/main) file.

<AlertBox>
The `main.ts` file is the entry file of a Rasengan.js application, so it is required.
</AlertBox>
