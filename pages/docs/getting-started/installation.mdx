...
- [`dev`](/docs/api/cli#development): runs `rasengan dev` to start Rasengan.js in development mode.
- [`build`](/docs/api/cli#build): runs `rasengan build` to build the application for production usage.
- [`prepare`](/docs/api/cli#prepare): runs `rasengan prepare` to setup configuration needed for hosting your application.
- [`start`](/docs/api/cli#start): runs `rasengan start` to start a Rasengan.js production server.
