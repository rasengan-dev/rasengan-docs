...
<Tabs items={["npx", "npm", "yarn"]}>
  <Tabs.Tab>
    ```bash filename="terminal" copy npx create-rasengan@latest ```
  </Tabs.Tab>
  <Tabs.Tab>```bash filename="terminal" copy npm create rasengan ```</Tabs.Tab>
  <Tabs.Tab>```bash filename="terminal" copy yarn create rasengan ```</Tabs.Tab>
</Tabs>
